...
def tapes_XP(params, state, local_hamils):
    ...
    for local_hamil in local_hamils:
        ...
        circuit_tape = qml.tape.QuantumTape(
            ops=circuit_tape,
            measurements=[MyMP(wires=local_hamil.wires, eigvals=local_hamil.eigvals())],
        )
...
def exp_val_XP(
    params=None, state=None, local_hamils=None, device=None, circuit_tapes=None
):
    ...
    ExpVal = qml.execute(circuit_tapes, device, gradient_fn=qml.gradients.param_shift)
...
def fidelity(theta, state_param, state_fixes, device):
    ...
    fids = qml.execute(circuit_tapes, device, gradient_fn=qml.gradients.param_shift)
